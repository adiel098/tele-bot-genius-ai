...
export async function createDockerContainer(botId: string, code: string, token: string): Promise<{ success: boolean; logs: string[]; containerId?: string; error?: string }> {
  initializeContainerState();
  ...
  try {
    ...
    const pythonBotScript = generatePythonBotScript(botId, containerId, token);
