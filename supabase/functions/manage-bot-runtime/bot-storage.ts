...
export async function loadBotCodeFromStorage(userId: string, botId: string): Promise<{ success: boolean; code?: string; logs: string[] }> {
  ...
  try {
    logs.push(BotLogger.log(botId, 'Loading bot code from Modal volume...'));
    
    // Call Modal FastAPI service to get bot files
    ...
    if (!response.ok) {
      logs.push(BotLogger.logError('Failed to fetch files from Modal: ' + response.statusText));
    ...
    logs.push(BotLogger.log(botId, 'SUCCESS: Loaded main.py from Modal volume: ' + botCode.length + ' characters'));
    ...
    logs.push(BotLogger.logError('Error loading bot code from Modal: ' + error.message));
    return { success: false, logs };
  }
}
